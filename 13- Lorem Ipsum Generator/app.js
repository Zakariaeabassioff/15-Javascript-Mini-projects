...
//Acceder a la zone ou va s'afficher le texte
const result = document.querySelector('.text');

//Acceder au bouton permettant le submit
const btn = document.querySelector('.btn');

btn.addEventListener('click', function(){
    let userAmount = parseInt(amount.value);//console.log(userAmount);

    //empty value
    //negative value
    //more than nine

    if(isNaN(userAmount) || userAmount < 0 || userAmount > 9){
        result.innerHTML = text[Math.floor(Math.random()*9)];
    } else {
        let tempText = text.slice(0, userAmount);
        tempText = tempText.join('<br>');
